...
export function renderBuilderItemHover(model) {
  ...
  if (model.hasHeroicEffects) sections.push(`<section class="is-heroic"><h3>Heroic Effects</h3>${model.heroicEffects.map((row) => `<div class="tlb-hc-heroic"><span>${esc(row.groupNumber)}</span><b>${esc(row.name)}${row.levelKnown ? ` · Lv ${esc(row.level)}` : " · level unknown"}</b><i></i><strong>${esc(row.value)}</strong></div>`).join("")}</section>`);
