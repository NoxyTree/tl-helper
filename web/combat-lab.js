...
import { loadCombatLabData, mapDisplayedLevel, OUTCOMES, projectAbilityRange, TIER_MAPPINGS } from "./combat-lab-model.js";

const byId = (id) => document.getElementById(id);
const ui = Object.fromEntries(["game-build","fatal-error","source-build","source-summary","target-build","target-summary","ability","component","tier","level","level-note","outcome","outcome-note","damage-source","damage-min","damage-max","result-title","result-range","expression","precision-grid","warnings","trace","provenance"].map((id) => [id, byId(id)]));
...
function bindEvents() {
  ui.ability.addEventListener("change", () => { populateComponents(); render(); });
  ui.component.addEventListener("change", render);
  ...
  ui["source-build"].addEventListener("change", () => { updateBuildSummaries(); prefillDamage(); render(); });
  ui["target-build"].addEventListener("change", () => { updateBuildSummaries(); render(); });
