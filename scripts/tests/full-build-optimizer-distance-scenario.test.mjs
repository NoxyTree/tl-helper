...
test("Full Build Optimizer creates one strict scenario for requests, results, hover, and tuning", () => {
  assert.match(html, /import \{ formatOptimizerScenario, optimizerScenarioOptions, parseOptionalPercentageBps \} from "\.\/optimizer\/full-build-optimizer\.js"/);
  ...
  assert.match(html, /function request\(\) \{ const heroicPolicy=.*return \{ build:state\.build, sourceKind:state\.source, \.\.\.scenarioRequestFields\(\)/);
