...
test("Gear Viewer target-distance scoring is explicit and disabled by default", () => {
  ...
  assert.match(html, /core\.createTargetDistanceScenario\(scoringContext\(\)\.build, state\.scenarioDistance\)/);
});
...
test("Gear Viewer scenario state has isolated cache identity and shareable persistence", () => {
  ...
  assert.match(html, /if \(state\.scenarioEnabled\) params\.set\("distance", String\(state\.scenarioDistance\)\)/);
  assert.match(html, /if \(params\.has\("distance"\)\)/);
  assert.match(html, /scenarioEnabled: state\.scenarioEnabled, scenarioDistance: state\.scenarioDistance/);
  assert.match(html, /target-distance effects at \$\{state\.scenarioDistance\}m/);
});
