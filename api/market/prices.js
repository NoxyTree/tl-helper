...
export default async function handler(request, response) {
  ...
  try {
    const url = new URL(request.url, `https://${request.headers.host ?? "tlhelper.org"}`);
  ...
  } catch (error) { return send(response, Number(error?.status ?? 500), { error: String(error?.message ?? error) }, "no-store"); }
}

function send(response, status, body, cacheControl) {
  response.setHeader("Cache-Control", cacheControl); response.setHeader("Content-Type", "application/json; charset=utf-8"); response.setHeader("X-Content-Type-Options", "nosniff");
