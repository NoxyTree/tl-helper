...
export function createTldbMarketService({ fetchImpl = fetch, now = () => Date.now(), timeoutMs = 12_000 } = {}) {
  let cached = null;
  ...
  return Object.freeze({
    async get(query) {
      ...
      if (!snapshot || time - snapshot.cachedAtMs > FRESH_TTL_MS) {
        try { snapshot = await fetchSnapshot({ fetchImpl, now, timeoutMs }); cached = snapshot; }
        catch (error) { if (!cached || time - cached.cachedAtMs > STALE_TTL_MS) throw error; snapshot = cached; stale = true; }
      }
      let catalog = cachedCatalog;
      if (!catalog || time - catalog.cachedAtMs > CATALOG_TTL_MS) {
        try { catalog = await fetchCatalog({ fetchImpl, now, timeoutMs }); cachedCatalog = catalog; }
        catch (error) { if (!cachedCatalog || time - cachedCatalog.cachedAtMs > CATALOG_STALE_TTL_MS) throw error; catalog = cachedCatalog; }
      }
...
async function fetchCatalog({ fetchImpl, now, timeoutMs }) {
  const response = await fetchImpl(CATALOG_URL, { headers: { accept: "application/json", "user-agent": "TL Helper market adapter" }, signal: AbortSignal.timeout(timeoutMs) });
  ...
  let rows;
  try { rows = JSON.parse(text)?.result?.data; } catch { throw new MarketUpstreamError("Questlog market catalogue returned invalid JSON."); }
  ...
  for (const row of rows) {
    ...
    byAuctionId.set(auctionId, item); byItemKey.set(itemKey, item);
  }
  return Object.freeze({ byAuctionId, byItemKey, cachedAtMs: now() });
}

async function fetchSnapshot({ fetchImpl, now, timeoutMs }) {
  const response = await fetchImpl(SOURCE_URL, { headers: { accept: "application/json", "user-agent": "TL Helper market adapter" }, signal: AbortSignal.timeout(timeoutMs) });
  if (!response.ok) throw new MarketUpstreamError(`TLDB market feed failed (${response.status}).`);
  if (Number(response.headers?.get?.("content-length") ?? 0) > MAX_RESPONSE_BYTES) throw new MarketUpstreamError("TLDB market feed exceeded the 2 MB safety limit.");
  const text = await response.text();
  if (text.length > MAX_RESPONSE_BYTES) throw new MarketUpstreamError("TLDB market feed returned oversized data.");
  let raw;
  try { raw = JSON.parse(text); } catch { throw new MarketUpstreamError("TLDB market feed returned invalid JSON."); }
  validateSnapshot(raw);
  return Object.freeze({ raw, cachedAtMs: now(), upstreamLastModified: response.headers?.get?.("last-modified") ?? null, upstreamEtag: response.headers?.get?.("etag") ?? null });
}

function validateSnapshot(raw) {
  if (!raw || typeof raw !== "object" || !Number.isFinite(raw.baseTime) || !raw.list || typeof raw.list !== "object") throw new MarketUpstreamError("TLDB market feed did not match the expected schema.");
  for (const { id } of Object.values(MARKET_REGIONS)) if (!raw.list[id] || typeof raw.list[id] !== "object" || Array.isArray(raw.list[id])) throw new MarketUpstreamError(`TLDB market feed omitted region ${id}.`);
}
...
function selectMarketData(snapshot, { region, itemId, itemKey }, { stale, servedAtMs, catalog }) {
  ...
  const metadata = Object.freeze({
    schema: "tl-helper.market-prices", schemaVersion: 1, source: "tldb", sourceUrl: SOURCE_URL,
    region, regionId: regionInfo.id, regionName: regionInfo.name,
    snapshotGeneratedAtUtc: new Date(snapshot.raw.baseTime * 1000).toISOString(), servedAtUtc: new Date(servedAtMs).toISOString(),
    upstreamLastModified: snapshot.upstreamLastModified, upstreamEtag: snapshot.upstreamEtag, catalogueSource: "questlog", stale,
  });
  ...
  const items = Object.entries(sourceRows).map(([id, row]) => normalizeItem(id, row, false, catalog.byAuctionId.get(id))).sort((left, right) => Number(left.itemId) - Number(right.itemId));
  return Object.freeze({ ...metadata, itemCount: items.length, items });
}

function normalizeItem(itemId, row, includeSales, catalogueItem) {
  if (!row || typeof row !== "object" || !Number.isFinite(row.price) || !Number.isFinite(row.quantity)) throw new MarketUpstreamError(`TLDB market item ${itemId} had an invalid price record.`);
  const item = { itemId, itemKey: catalogueItem?.itemKey ?? null, name: catalogueItem?.name ?? null, minimumPrice: row.price, quantity: row.quantity };
  if (includeSales) item.listings = Array.isArray(row.sales) ? row.sales.flatMap((listing) => {
    const quantity = Number(listing?.c), expiresAt = Number(listing?.e), price = Number(listing?.p);
    return Number.isFinite(quantity) && Number.isFinite(expiresAt) && Number.isFinite(price) ? [{ quantity, expiresAtUtc: new Date(expiresAt * 1000).toISOString(), price }] : [];
  }) : [];
  return Object.freeze(item);
}

export class MarketRequestError extends Error { constructor(message, status = 400) { super(message); this.name = "MarketRequestError"; this.status = status; } }
export class MarketUpstreamError extends Error { constructor(message) { super(message); this.name = "MarketUpstreamError"; this.status = 502; } }
